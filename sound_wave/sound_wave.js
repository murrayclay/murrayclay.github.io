...
function Ref_Particle(x, y, radius, color, strokecolor=color, vel_amp = 8/10.) {
    this.x = x;
    this.y = y;
...
function init() {
    ...
    wavewidth = (canvas.width-2.*radius)/6.
    for (let i = 0; i < nparticles; i++) {
	let x = randomIntFromRange(radius+wavewidth, canvas.width - radius);
	let y = randomIntFromRange(radius, canvas.height - radius);
	const color = colors[3];
	const strokecolor = colors[2];
	const alt_color = colors[0];
	const alt_strokecolor = colors[1];

	if (i !== 0) {
	    for (let j = 0; j < particles.length; j++) {
		if (distance(x, y, particles[j].x, particles[j].y) - radius*2 < 0) {
		    x = randomIntFromRange(radius+wavewidth, canvas.width - radius);
	...
	/*if (i == 0) {
	    particles.push(new Particle(x, y, radius, alt_color, alt_strokecolor, vel_amp, explode));
	} else {*/
    ...
    // make particles on the left side with larger thermal velocities
    wavewidth = (canvas.width-2.*radius)/6.
    for (let i = 0; i < nparticles/3; i++) {
	let x = randomIntFromRange(radius, radius + wavewidth);
	let y = randomIntFromRange(radius, canvas.height - radius);
	const color = colors[3];
	const strokecolor = colors[2];
	const alt_color = colors[0];
	...
	if (i !== 0) {
	    for (let j = 0; j < particles.length; j++) {
		if (distance(x, y, particles[j].x, particles[j].y) - radius*2 < 0) {
		    x = randomIntFromRange(radius, radius + wavewidth);
		    y = randomIntFromRange(radius, canvas.height - radius);
...
/*	if (i == 0) {
	    particles.push(new Particle(x, y, radius, alt_color, alt_strokecolor, vel_amp*3., explode));
	} else {*/
	    particles.push(new Particle(x, y, radius, color, strokecolor, vel_amp*3., explode));

	//}
    }

    // make a reference particle moving at the average sound speed
    ref_particles.push(new Ref_Particle(1+radius,canvas.height-radius-1, radius, colors[1], colors[1], vel_amp*3., explode));
     
}

var myButton = document.getElementById("reset_button");
...
function animate() {
    ...
    ref_particles.forEach(ref_particle => {
	ref_particle.update(ref_particles);
    });
}
