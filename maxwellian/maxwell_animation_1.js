...
addEventListener('change', function() {
    //wave.amplitude = document.getElementById("vel_slider").value;
    vel_amp = document.getElementById("vel_slider_1").value/10.0;  /* /3.0 */
...
// Objects
function Particle(x, y, radius, color, strokecolor=color, vel_amp = 8/10., explode=0) { /* 5 */
    this.x = x;
    this.y = y;
	...
	//x: (Math.random() - 0.5) * vel_amp,
	//y: (Math.random() - 0.5) * vel_amp
    };
    this.radius = radius;
    this.color = color;
    ...
    const xnorm = (this.x-canvas.width/4.0)/(canvas.width/2.0);
	...
	if (this.x - this.radius <= 0 || this.x + this.radius >= canvas.width/2) {
...
function init() {
    ...
    axes = {
	x0: canvas.width*(7/12),
	y0: canvas.height/3,
	xlen: canvas.width/3,
	ylen: canvas.height/2
    }
...
function animate() {
    ...
    // Draw the outlines separating the parts of the canvas
    c.beginPath();
	...
	vels.push(Math.sqrt(Math.pow(particle.velocity.x,2) + Math.pow(particle.velocity.y,2)));  // gets velocities for the histogram
    });

    // draw the axes
    x0 = canvas.width*7/12.
    y0 = canvas.height*0.6
    xmax = canvas.width*11/12.
    ymax = canvas.width*0.1
    ...
    nbins = 50;
    binmax = vrange; //vel_amp*3.0;
    bins = histogram(vels, nbins, min=0.0, max=binmax);

    drawwidth = (xmax-x0)/nbins*binmax/vrange;

    // RMS velocity in 2D since this is a 2D "gas"
    v0 = vel_amp/Math.sqrt(2.0)

    // to yet the y normalization looking good
    norm = -(ymax - y0)/3.;

    normhist = norm/nparticles*15.; 
    
    c.save()
    c.fillStyle = colors[2];
    for (let i = 0; i < bins.length; i++) {
	c.fillRect(x0+i*drawwidth, y0, drawwidth, -bins[i]*normhist);
    }
    c.restore()

    // draw the Maxwellian velocity distribution   
    c.beginPath();
    c.moveTo(x0, y0);

    for (let i = 0; i < xmax-x0; i++) {
	v = i/(xmax-x0)*vrange;
	// 2D Maxwellian:
	c.lineTo(x0 + i, y0-norm*Math.pow(1./v0/v0/2./3.14159, 2./2.)*2.*3.14159*v*Math.exp(-v*v/2./v0/v0));
    }
    c.lineWidth = 2.0
