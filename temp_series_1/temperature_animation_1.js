...
function randomRealFromRange(min,max) {
...
function init() {
	...
	vel_amp_pick = vel_amp*Math.exp(randomRealFromRange(-3,0));
	particles.push(new Particle(x, y, radius, color, strokecolor, vel_amp_pick, explode));
    }
    if (explode == 1) {
	explode = 0;
    }
